...
(() => {
    ...
    /* ── Main update ──────────────────────────────────────────── */
    function updateUI(d) {
        setText(el.connStatus, 'CONNECTED');
        ...
        if (prev.rpm !== rpm) { Gauges.drawRPMGauge(rpmCanvas, rpm, 5000); prev.rpm = rpm; }

        /* Speed gauge */
        const speed = d.vehicle_speed || 0;
        const si = Math.round(speed);
        setText(el.speedValue, si);
        if (prev.spd !== si) { Gauges.drawSpeedometer(speedCanvas, speed, 60); prev.spd = si; }
    ...
    function updateTimers() {
        const now = new Date();
        setText(el.clock, pad2(now.getHours()) + ':' + pad2(now.getMinutes()) + ':' + pad2(now.getSeconds()));
        const elapsed = Math.floor((Date.now() - startTime) / 1000);
        setText(el.session, pad2(Math.floor(elapsed / 60)) + ':' + pad2(elapsed % 60));
    }
    ...
    /* ── Init ─────────────────────────────────────────────────── */
    Gauges.drawSpeedometer(speedCanvas, 0, 60);
